...
export class ReportGenerator {
    ...
    private processFeatures(detectedFeatures: DetectedFeature[]): Map<string, FeatureUsage> {
        ...
        for (const feature of detectedFeatures) {
            ...
            if (!featureUsageMap.has(featureId)) {
                const featureDetails = this.compatibilityService.getFeatureDetails(featureId);
                if (!featureDetails) {
                    continue; // Skip features without details
                }
    ...
    private generateReportSummary(projectResult: ProjectAnalysisResult, features: FeatureUsage[]): ReportSummary {
        ...
        return {
            ...
            widelyAvailable: projectResult.summary.widelyAvailable,
            newlyAvailable: projectResult.summary.newlyAvailable,
            limitedAvailability: projectResult.summary.limitedAvailability,
            riskDistribution,
            fileTypeBreakdown
        };
    }
